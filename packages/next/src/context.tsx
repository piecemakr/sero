...
export const SeroNextProvider: React.FC<SeroNextProviderProps> = ({ children, options }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (!pathname) {
        console.warn(
          'Sero: usePathname returned null. Make sure you\'re using Next.js App Router and this component is rendered on the client side.'
        );
      }
      ...
      if (options?.viewTransitions !== false && !('startViewTransition' in document) && !configuredRef.current) {
        console.info(
          'Sero: View Transitions API is not supported in this browser. Transitions will work but without the native API.'
        );
      }
    }
  }, [options, pathname]);
  ...
  const contextValue: SeroNextContextValue = useMemo(() => ({
...
export const useSeroNext = (): SeroNextContextValue => {
  ...
  if (!context) {
    throw new Error(
      'useSeroNext must be used within a SeroNextProvider. ' +
